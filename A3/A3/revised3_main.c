...
int* Parallel_Qsort(MPI_Comm curr_group, int rank, int size,  int* local_arr, int n, int depth){
	...
	if (rank < group_size) {
		...
		return Parallel_Qsort(nextgroup, newrank, group_size/2, merged_arr, newlen, depth+1);
